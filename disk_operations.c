...
static void start_global_states(); 
...
static void write_free_slots_page_struct(FreeSlotsPage* fs, Page p) {
	...
	FILE* file = fopen("mydatabase.db", "rb+");
	if (file == NULL) {
...
static void write_meta_data(DB_Header* db) {
	FILE* file = fopen("mydatabase.db", "rb+");
	if (file == NULL) {
	...
	size_t write_check = fwrite(db, sizeof(DB_Header), 1, file);
	if (write_check != 1) {
...
static DB_Header* create_meta_data() {
...
static void free_meta_data_struct(DB_Header* db) {
	...
	write_meta_data(db); // Write to disk just incase
	free(db);
}
...
void start_database_page() {
	...
	file = fopen("mydatabase.db", "wb+");
	if (file == NULL) {
		perror("\033[1;31mError creating database file \033[0m");
		return;
	}

	db_information = create_meta_data();
	...
	write_meta_data(db_information);
...
static void start_global_states() {
	...
	// If file exists but meta data is NULL, have to read in last sessions meta data into struct
	if (db_information == NULL) {
		db_information = create_meta_data();
		if (db_information == NULL) {
			fclose(file);
...
void save_database() {
	...
	if (file != NULL) {
		if (db_information != NULL) {
			db_information->current_record_page = current_record->page_id;
			db_information->free_pages_tail = free_tail;
			db_information->free_slots_pages_tail = slots_tail;
