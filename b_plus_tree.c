...
/* PUBLIC API'S TO USE THE B+TREE */
...
static void btree_leaf_remove_helper(B_Tree* b, LeafNode* n, Page n_page) {
	    ...
	    ((LeafNode *)get_page(parent->pages[right_idx], READ))->count > LEAF_MIN) 
	{
		...
		parent->keys[child_idx] = sibiling->entries[0].key; // sibilings new lowest entry key becomes seperator key
		return;
	}
	         ...
	         ((LeafNode *)get_page(parent->pages[left_idx], READ))->count > LEAF_MIN) 
	{
		...
		parent->keys[child_idx - 1] = n->entries[0].key; // Sibilings old highest entry becomes the seperator
		return;
	}
